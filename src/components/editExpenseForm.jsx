...
import { editExpenseAsync, getexpenseAsync } from '../redux/slices/expenseSlice/expenseThunk';
...
const EditExpenseForm = ({ expenseId, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (!expense) {
          dispatch(getexpenseAsync({ id: expenseId }))
            .then((response) => {
              const fetchedExpense = response.payload;
              console.log('Fetched Expense:', fetchedExpense);
              setAmount(fetchedExpense.amount);
              setCategory(fetchedExpense.category);
              setAccount(fetchedExpense.account || ''); // Ensure account is set or use a default value
              setDescription(fetchedExpense.description);
              setDate(fetchedExpense.date.split('T')[0]); // Format the date
            })
            .catch((error) => {
    ...
    const handleSubmit = (e) => {
      ...
      try {
        dispatch(editExpenseAsync({ id: expenseId, ...expenseData }));
        onClose();
      } catch (error) {
  ...
  return (
    <div className="w-5/12 rounded-md max-w-5xl mx-auto bg-white shadow-lg border border-zinc-100 mt-4 p-4">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mt-4 text-sm text-teal-800 font-light text-center">Edit Transaction</div>
        ...
        <div className="mb-4">
          ...
          <select id="category" value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
            ...
            <option value="660c02f2ae0c2fcb1121ba29">Food</option>
            <option value="660c02f2ae0c2fcb1121ba30">Utilities</option>
          </select>
        </div>
